...
const buildPurchasesFromSoldTickets = (
  eventId: string,
  soldTickets: SoldEventTicketItem[],
): TicketPurchase[] => {
  const groupedPurchases = new Map<string, SoldEventTicketItem[]>();

  soldTickets.forEach((ticket) => {
    const purchaseId = ticket.ticket_purchase.id;
    groupedPurchases.set(purchaseId, [
      ...(groupedPurchases.get(purchaseId) ?? []),
      ticket,
    ]);
  });

  return [...groupedPurchases.values()].map((tickets) => {
    const firstTicket = tickets[0];
    const purchase = firstTicket.ticket_purchase;
    const itemGroups = new Map<
      string,
      {
        ticketType: SoldEventTicketItem['ticket_type'];
        quantity: number;
      }
    >();
    ...
    return {
      ...
      items: [...itemGroups.entries()].map(([ticketTypeId, group]) => {
        const unitPrice = Number(group.ticketType.price ?? 0);
        return {
          id: `${purchase.id}-${ticketTypeId}`,
          ticket_purchase_id: purchase.id,
          ticket_type_id: ticketTypeId,
          quantity: group.quantity,
          unit_price: Number.isFinite(unitPrice) ? unitPrice : 0,
          currency: group.ticketType.currency,
          ticket_type_label: group.ticketType.name,
          created_at: purchase.created_at,
          ticket_type: {
            id: ticketTypeId,
            event_id: eventId,
      ...
      tickets: tickets.map((ticket) => ({
        ...
        attendee_firstname: ticket.attendee_firstname,
        attendee_lastname: ticket.attendee_lastname,
...
interface OrganizerTicketTransactionsProps {
  user: User;
}
...
const OrganizerTicketTransactions = ({ user }: OrganizerTicketTransactionsProps) => {
  ...
  const loadOverview = useCallback(async () => {
    ...
    const purchasesByEvent = await Promise.all(
      organizerEvents.map(async (event) => {
        ...
        return buildPurchasesFromSoldTickets(event.id, response.tickets);
      }),
    );
  ...
  const refundedTickets = useMemo(
    () =>
      displayedPurchases
        .filter((purchase) => String(purchase.status ?? '').toLowerCase() === 'refunded')
        .reduce((sum, purchase) => sum + purchase.tickets.length, 0),
    [displayedPurchases],
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        />
        <StatCard
          label="Tickets remboursés"
          value={String(refundedTickets)}
          helper="Tickets liés aux transactions remboursées"
        />
      </section>
...
export default OrganizerTicketTransactions;

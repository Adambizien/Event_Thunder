...
interface OrganizerSocialPostsProps {
  user: User;
}

const OrganizerSocialPosts = ({ user }: OrganizerSocialPostsProps) => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setEvents(loadedEvents.filter((event) => event.creator_id === user.id));
      setPosts(normalizeExpiredStatus(loadedPosts.filter((post) => post.user_id === user.id)));
      setError(null);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    void loadData();
  }, [hasActiveSubscription, loadData]);
    ...
    isGracePeriod && gracePeriodEnd ? (
      <div className="rounded-2xl border border-amber-500/40 bg-amber-500/15 p-6 shadow-2xl backdrop-blur-lg">
        ...
        <p className="mt-2 text-amber-100/90">
          Vous ne serez bientôt plus abonné et devrez renouveler l'abonnement, mais vous gardez l'accès à la gestion des posts pour l'instant.
        </p>
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="rounded-2xl border border-amber-500/40 bg-amber-500/15 p-6 shadow-2xl backdrop-blur-lg">
          ...
          <p className="mt-2 text-amber-100/90">
            Vous devez être abonné à un plan actif pour accéder à la gestion des posts.
          </p>
          <Link
            to="/subscription"
...
export default OrganizerSocialPosts;